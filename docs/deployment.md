...
- работающий Traefik с сетью `vi-common_vi`, entrypoints `http` и `https`;
...
Общий Traefik автоматически запросит сертификат после того, как DNS начнет
указывать на сервер. Проверка:
